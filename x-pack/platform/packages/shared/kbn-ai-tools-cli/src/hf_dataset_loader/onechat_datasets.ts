...
export async function createOneChatDatasetSpec(
  ...
  return {
    ...
    revision: 'main',
    index: datasetName.replace(/\//g, '-'), // Replace slashes with dashes for valid index name
    mapping: {
      properties: mapping.mappings.properties,
      _meta: mapping.mappings._meta,
    },
    mapDocument: (r) => {
      // Just return the parsed CSV data as-is
      logger.info(`Mapping document for OneChat dataset: ${datasetName}. Srdjan`);
      logger.info('Srdjan kaze data je ' + JSON.stringify(r));
